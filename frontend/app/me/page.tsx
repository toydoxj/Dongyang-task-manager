...
function TodayTasks({
  ...
  // 분류별 그룹 (정의 순서대로 표시, 미분류는 마지막에 강조)
  ...
  for (const t of open) {
    const key = (t.category && CATEGORY_ORDER.includes(t.category as never))
      ? t.category
      : "__none__";
    grouped.get(key)!.push(t);
  }

  const sections: Array<{ key: string; label: string; items: Task[]; warn: boolean }> = [];
  for (const c of CATEGORY_ORDER) {
    const items = grouped.get(c)!;
    if (items.length > 0) sections.push({ key: c, label: c, items, warn: false });
  }
  const noneItems = grouped.get("__none__")!;
  if (noneItems.length > 0) {
    sections.push({
      key: "__none__",
      label: "미분류 — 분류를 지정해 주세요",
      items: noneItems,
      warn: true,
    });
  }
    ...
    <div className="space-y-4">
      {sections.map((sec) => (
        <div key={sec.key}>
          ...
          >
            <span>{sec.label}</span>
            <span className="text-zinc-400">({sec.items.length})</span>
          </h3>
