...
- 손상 데이터 복구법 (2026-05-27 적용): 노션 원본은 안 망가지므로 `notion.get_page(page_id)` → `sync.upsert_page(kind, page)`로 mirror를 노션 read-포맷으로 재조회 갱신. 노션 원본 불변·되돌릴 수 있음. 복구 범위 = project 3 + task active 3건이었음 (task archived 33건은 보관용이라 제외)
- **진단 주의 — write-포맷 properties 자체는 손상 지표가 아니다.** PR-FX read fallback 적용 후에는 write helper(`_title`/`_rich_text`)가 여전히 plain_text 없는 write-포맷을 mirror에 저장해도 read 시 정상 추출됨. 따라서 손상 판별은 `jsonb`에 plain_text 유무가 아니라 **"mirror 파생 컬럼(code/name/title)이 빈 값인데 properties content는 살아있음"**으로 해야 한다. 후자가 0이면 표시/집계 정상
- 근본 정합화 (PR-FY, 2026-05-27): read fallback(PR-FX)은 증상 방어. "노션=백업 / mirror=작업 DB" 원칙상 mirror 데이터 자체가 정상 포맷이어야 하므로 write 시점에 정규화. `notion_props.normalize_properties_for_mirror(props)` 추가 → `sync._upsert_one`(단일 dispatch — upsert_page/upsert_in_session + incremental sync 전 구간) + `seal_request_mirror.apply_update_to_mirror`(sync 비경유 별도 경로)에 적용. top-level title/rich_text seg에 plain_text 보강(idempotent·원본불변·mention skip). **outbox payload(노션 push)는 별도 dict라 무관** — 노션엔 plain_text 보내지 않음. `_upsert_seal_request`/`_upsert_suggestion`/`_upsert_client`의 자체 plain_text 루프는 정규화된 props를 받아 자동 정상화(루프 교체 불필요)
- mirror-direct write 도메인 = tasks·projects·seal_requests(enqueue 패턴). 노션 직접 write(sales·suggestions·clients)는 노션 응답 저장이라 write-포맷 무발생. 신규 도메인을 mirror-direct로 cut-over 시 정규화 경로(_upsert_one 또는 별도 helper) 통과 여부 확인 필수
