...
| **PR-CF Phase 4-J 4단계 — quote/preview를 quote_meta에 통합** | `POST /quote/preview`(견적 산출 read-only)를 `routers/sales/quote_meta.py`로 이동. `QuoteResult` 고아 import 정리. 단일 endpoint라 helper 의존 없음. `__init__.py` 930 → ~920 lines | (pending push) |
