...
| **PR-BX partial_errors schema (외부 리뷰 12.x #1 본격)** | seal_requests 응답에 `partial_errors: [{code, target, message, retryable}]` 정형 필드 추가. create_seal_request + attachments endpoint에서 `failed[]` → `partial_errors[]` 변환(드라이브 업로드 실패 정형화). frontend `PartialError` 타입 + `SealRequestItem.partial_errors?` 추가. **frontend toast 노출은 별도 cycle**(타입만 정의). | (pending push) |
