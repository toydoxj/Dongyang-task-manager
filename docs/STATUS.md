...
> 마지막 업데이트: 2026-05-16 (Phase 4-G PR-EM/EN 운영 회귀 → 즉시 revert, 1단계 상태로 복귀)
...
| **PR-EM 4-G 2단계 2차 — frontend cookie-only 인증 전환 (PR-BP+BQ 통합 재시도)** | `saveAuth(token, user)` token 인자 무시(signature backward-compat 유지로 chunk stale 회피, 체크리스트 #2). `isLoggedIn` 을 `!!getUser()` 로 변경 — cookie httpOnly이라 frontend read 불가, user JSON 존재로 판단. AuthGuard.refresh isLoggedIn 분기에 `verifyAndHydrateFromMe` 추가 — cookie validity 검증, 200이면 user 갱신, 401/network는 graceful fallback(Phase 0-B 정책 + PR-BO silent SSO retry 자동 회복). vitest 32 통과(saveAuth 시나리오 PR-EM 동작 반영). **기존 사용자: localStorage token 그대로 유지 + backend header fallback 호환. 신규 로그인부터 cookie 단독 자연 수렴.** PR-CY callback page raw fetch 패턴 + AuthGuard에서 동일 패턴 활용으로 INCIDENT #4 무한 재귀 회피 | 9f5576e (revert: 7ea0824) |
| **PR-EN 4-G 2단계 3차 — authFetch Authorization header 첨부 코드 완전 제거** | `getToken` 제거, `authFetch`/`backendLogout`에서 Authorization 첨부 라인 제거. vitest 33 통과. **운영 deploy 후 즉시 cookie 미발급 사용자 401 무한 회귀 발생 → PR-EM/EN 모두 revert.** 회귀 원인: PR-EM이 saveAuth token 저장 중단 → PR-EN으로 header 첨부도 차단 → cookie 발급 안 된 사용자가 backend header fallback 호환성도 잃음. 사고 기록은 INCIDENT.md / error.md | b60d720 (revert: b752a40) |
| **PR-EO 4-G 5차 안전망 1차 — verifyAndHydrateFromMe reason 시그니처** | INCIDENT #6 1차 충족. `VerifyResult` discriminated union 추가 (`reason: 'ok'\|'unauthorized'\|'network'`) → 호출처가 401(cookie 미발급) vs network 실패 차등 처리 가능. callback page는 결과 무시 흐름이라 호환. vitest 4 시나리오(ok/401/500=unauthorized/network) 통과. AuthGuard 활용은 5차 본격 PR로 분리 | feb2a4c |
| **PR-EP 4-G 5차 안전망 2차 — callback page playwright e2e cookie 시나리오** | INCIDENT #5 충족. `e2e/auth-callback.spec.ts` 신설 — 3 시나리오(200 user 갱신 / 401 graceful warn + redirect / network fail graceful warn + redirect). `_helpers.ts`에 `makeCallbackFragment` + `mockAuthMe` helper. 누적 e2e 8 통과(7.2s). PR-EM/EN 사고 모델(401 무한)을 자동 검출 가능 | cba951c |
| **PR-EQ PR-CR 진단 1차 액션 — list_seal_requests notion filter push down** | `routers/seal_requests/list_endpoint.py`: project_id 있으면 notion relation `contains` filter dict로 push down → 노션 서버 단에서 매칭 페이지만 반환. 옛 query_all 전체 fetch + Python in-memory filter 폐기. 프로젝트 상세 진입(빈번)에서 fetch 데이터/대기 시간 감소. mirror_seal_requests는 pending-count 전용 schema 그대로 유지(본격 마이그레이션은 별도 큰 PR). pytest 63 통과. unused `notion_props as P` import 제거 | 5e1794f |
| **PR-ER PR-CR 진단 2순위 — cashflow list payer+contract_item batch** | list 경로 전용 `_resolve_cashflow_labels_batch` 신설 — payer/contract_item label 조회를 `UNION ALL`로 1 round-trip 통합. Render→Supabase round-trip 100-200ms 절감. 단일 entry endpoint(POST/PATCH)는 기존 `_resolve_payer_names` + `_resolve_contract_item_labels` 그대로 유지 (item 1개라 효과 없음, 단순성 우선). Codex 권고 — list 경로만 union_all 적용. pytest 63 통과 | (push 대기) |
...
| **4-G** JWT localStorage → httpOnly cookie | 1단계 완료(PR-BH) / 2단계 4차 시도 회귀 → 즉시 revert (PR-EN b752a40, PR-EM 7ea0824, 2026-05-16). PR-EL telemetry(93f4d5b)는 유지. cookie 발급 안 된 사용자에서 401 무한 사고 — header fallback 안전망 끊은 게 원인. **#5 충족: PR-EP(cba951c) — callback page 3 시나리오 e2e. #6 1차 충족: PR-EO(feb2a4c) — VerifyResult reason 시그니처.** 잔여: AuthGuard reason 활용 + telemetry 운영 1주 cookie 비율 99%+ 확인 → 5차 재시도 가능 | XSS 방어 강화 |
