...
| **PR-BP /me hydration (INCIDENT 체크리스트 #4)** | `hydrateUserFromMe()` helper — `/api/auth/me` 호출, 응답을 `saveAuth(user)`로 갱신. SSO callback page useEffect에서 redirect 직전 호출 → backend가 검증한 user 정보로 localStorage 갱신 + cookie/token validity 동시 검증. 실패 시 fragment user fallback(graceful, PR-BO silent 재시도가 후속 회복) | (pending push) |
