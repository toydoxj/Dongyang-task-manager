...
| **PR-BP /me hydration (INCIDENT 체크리스트 #4)** | `hydrateUserFromMe()` helper — `/api/auth/me` 호출, 응답을 `saveAuth(user)`로 갱신. SSO callback page useEffect에서 redirect 직전 호출 → backend가 검증한 user 정보로 localStorage 갱신 + cookie/token validity 동시 검증. 실패 시 fragment user fallback(graceful, PR-BO silent 재시도가 후속 회복) | bd86c9f |
| **PR-BQ Phase 4-G 2단계 재시도 — Authorization header 첨부 제거** | authFetch에서 `Authorization: Bearer <token>` 첨부만 제거 — cookie 단독 인증. 다른 변경 없음(localStorage token 저장 유지 / saveAuth signature 그대로 / callback fragment 그대로). 이전 PR-BI 대비 최소 변경 + INCIDENT 체크리스트 #2~#5 안전망 보강 위 진행. backend telemetry `auth_via_header`로 web 사용량 0 수렴 추적. dy-midas는 별도 client(cli=dy-midas)라 영향 없음 | (pending push) |
