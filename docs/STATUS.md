...
| **4-G** JWT localStorage → httpOnly cookie | 1단계 완료(PR-BH) / 2단계 4차 시도 회귀 → 즉시 revert (PR-EN b752a40, PR-EM 7ea0824, 2026-05-16). PR-EL telemetry(93f4d5b)는 유지. cookie 발급 안 된 사용자에서 401 무한 사고 — header fallback 안전망 끊은 게 원인. INCIDENT.md 체크리스트 #5(playwright e2e cookie 시나리오) + #6(graceful fallback 설계 개선)이 5차 시도 전 필수. **#6 1차 충족: PR-EO(feb2a4c, 2026-05-17) — VerifyResult reason 시그니처. 잔여: AuthGuard 활용 + #5 playwright** | XSS 방어 강화 |
