...
| **PR-BN saveAuth backward-compat (INCIDENT 체크리스트 #2)** | `saveAuth(token, user)` + `saveAuth(user)` 두 호출 시그니처 모두 지원하도록 overload + runtime branch. PR-BI 사고 원인 가설 #2(Vercel chunk 부분 stale로 옛/새 chunk 혼재 시 user 자리에 token 저장 → loop) 회피. 누적 vitest 21(saveAuth 3 시나리오 + clearAuth) | (pending push) |
