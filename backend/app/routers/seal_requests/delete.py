...
async def _sync_linked_task_bg(task_id: str, *, target: str) -> None:
    """사용자 응답과 분리해 연결 task를 best-effort 동기화."""
    from app.routers.seal_requests import _sync_linked_task

    await _sync_linked_task(get_notion(), task_id, target=target)
...
async def delete_seal_request(
    ...
    if seal_type == "구조검토서" and doc_no:
        is_last = SL.is_last_review_doc_number_from_mirror(db, doc_no=doc_no)
    ...
    if keep_with_marker:
        ...
        get_sync().archive_in_session(db, "seal_requests", page_id)
        enqueue(
            db,
            aggregate_type="seal_requests",
            aggregate_id=page_id,
    ...
    for tid in (item.linked_task_id, item.lead_task_id, item.admin_task_id):
        if tid:
            _spawn_task(_sync_linked_task_bg(tid, target="완료"))
