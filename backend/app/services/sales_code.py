...
_CODE_RE = re.compile(r"^(\d{2})-영업-(\d+)$")
...
def next_sales_code(db: Session, year_yy: int | None = None) -> str:
    """다음 {YY}-영업-{NNN} CODE 발급. advisory lock 보호.
    ...
    stmt = select(M.MirrorSales.code).where(M.MirrorSales.code.like(f"{prefix}%"))
