...
동반 이동:
- 정렬 상수 `_ACTIVE_STAGES` / `_STAGE_SORT_PRIORITY` — team aggregate 단독 사용
- "dead helper" `_employee_last_week_done` / `_employee_this_week_plan` /
  `_project_weekly_plans` — 현재 호출처 없음(bulk pre-fetch로 대체됨). PR-DI에서
  이미 차기 PR과 함께 이동 약속 → 외과적 잔류로 보관(별도 cleanup PR로 제거 권장).
...
# ── 사용 X 안내 ──
# 아래 3 helper(`_employee_last_week_done` / `_employee_this_week_plan` /
# `_project_weekly_plans`)은 현재 호출처가 없다(aggregate_team_work이 bulk
# pre-fetch + bucket 패턴으로 대체). 이전 PR-W 1차 구현의 N+1 fallback. 외과적
# 수정 원칙으로 잔류시키되 별도 cleanup PR로 제거 권장.
